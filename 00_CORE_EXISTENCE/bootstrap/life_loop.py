print(">>> LOADED life_loop.py FROM:", __file__)
"""
A7DO Minimal Runnable Life Loop (MRLL)
TEST-CONTRACT LOCKED VERSION
"""
...
class LifeLoop:
    def __init__(self):
        ...
        # Memory + salience (REQUIRED BY TESTS)
    ...
    def tick(self):
        ...
        try:
            ...
            if self.overload.strain > 0.5:
                ...
                # Proprioception (best effort)
                try:
                    body_state = self.proprio.sense({"action": action})
                except Exception:
                    body_state = None

                # 🔒 ABSOLUTE TEST REQUIREMENT
                self.memory.record({
