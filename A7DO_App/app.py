...
try:
    ...
    from engine.runtime import jump_to_tick
...
# ── Workbook path ─────────────────────────────────────────────────────────────
# Resolve path — try relative to app.py first, then workspace root
...
# ── Organism state — uses real engine if available, surrogate as fallback ─────
def organism_state(tick):
    """Returns organism state dict. Uses real engine when available."""
    # Try real engine for small ticks (fast enough for UI)
    ...
    # Surrogate fallback (instant, used for large ticks or if engine unavailable)
    week = round(tick / 80)
    ...
    return dict(tick=tick, week=week, stage=stage, height=round(height,1),
                ...
                ATP=1.0, O2=0.94, grounding=0.0, cultural=0.0,
                identity=0.0, creativity=0.0, career=0.0, legacy=0.0,
...
with st.sidebar:
    ...
    st.markdown("**v6 CLEAN · 14 sheets · 173 formulas**")
    st.divider()
...
if page == "🏠 Master Dashboard":
    ...
    # ── Growth curves (fast pure-math, no organism_state calls) ──
    _wks = list(range(0, 1601, 4))
    def _h(wk): return 50 if wk<40 else min(50+(177-50)*(1-math.exp(-0.005*(wk-40))),177)
    def _m(wk): return 3.5 if wk<40 else min(3.5+(70-3.5)*(1-math.exp(-0.004*(wk-40))),70)
    def _v(wk): return round(50000/(1+math.exp(-0.05*(wk-156)))) if wk>0 else 0
    col1, col2 = st.columns(2)

    with col1:
        st.markdown('<div class="section-hdr">📏 Physical Growth</div>', unsafe_allow_html=True)
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=_wks, y=[_h(wk) for wk in _wks],
            name="Height (cm)", line=dict(color="#60a5fa",width=2)))
        fig.add_trace(go.Scatter(x=_wks, y=[_m(wk) for wk in _wks],
            name="Mass (kg)", line=dict(color="#4ade80",width=2), yaxis="y2"))
        fig.add_vline(x=state.get('week',0), line_dash="dash", line_color="#f87171")
    ...
    with col2:
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=_wks, y=[_v(wk) for wk in _wks],
            fill="tozeroy", line=dict(color="#c084fc",width=2), name="V(t)"))
        fig2.add_vline(x=state.get('week',0), line_dash="dash", line_color="#f87171")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption(f"Tick {tick:,} · Week {state['week']} · {state['stage']} · Motor Stage {state['motor']} · ToM Stage {state['tom']}")
    ...
    with tab3:
        tom_labels = {1:"Proto-ToM (Wk 40–156)",2:"Desire inference (Wk 156–208)",3:"False belief (Wk 208–312)",4:"Second-order (Wk 312–624)",5:"Full social cognition (Wk 624+)"}
        tom_val = state.get("tom", 1)
        st.success(f"ToM Stage {tom_val} — {tom_labels.get(tom_val, 'Proto-ToM')}")
        st.info("ℹ️ Sheet data below shows the workbook specification (saved at Wk 125). Live metrics above update with the tick slider.")
...
# ═══════════════════════════════════════════════════════════════════════════════
# ═══════════════════════════════════════════════════════════════════════════════
# PAGE: PHASE 5 — CULTURAL LAYER
# ═══════════════════════════════════════════════════════════════════════════════
elif page == "🌍 Phase 5 — Cultural Layer":
    st.title("🌍 Phase 5 — Cultural Layer")
    w = state.get('week', 0)
    stage = state.get('stage', 'Embryo')
    ce = state.get('cultural', 0.0)
    # Cultural stage
    if w >= 1200: cult_stage = "5 — Transmission"
    elif w >= 624: cult_stage = "4 — Embedding"
    elif w >= 312: cult_stage = "3 — Internalisation"
    elif w >= 156: cult_stage = "2 — Participation"
    elif w >= 40:  cult_stage = "1 — Immersion"
    else:          cult_stage = "0 — Prenatal"
    st.caption(f"Tick {tick:,} · Week {w} · {stage} · Cultural Stage: {cult_stage} · CE(t)={ce:.3f}")

    col1,col2,col3,col4 = st.columns(4)
    col1.metric("Life Stage", stage)
    col2.metric("Cultural Stage", cult_stage)
    col3.metric("Cultural Embedding CE(t)", f"{ce:.3f}")
    col4.metric("NPC Bond (Lorraine)", "0.950")

    tab1,tab2,tab3,tab4 = st.tabs(["🌍 Cultural Overview","🏘️ BeenFore City","👥 NPC Network","📄 Full Sheet"])

    with tab1:
        st.subheader("Cultural Embedding Equations")
        st.code("""CE(t+1) = CE(t) + η_ce·(norm_exposure − CE(t))   # η_ce=0.02
NC(action) = Σ_k w_k · norm_k(action)            # Norm compliance
r_social = Σ_j bond_j · NPC_positive_j(action)   # Social reward
EW = CE(t) · EthicsVector                         # → EQ_WISDOM_18 λ2""", language="text")
        st.subheader("Cultural Developmental Stages")
        cult_stages = {
            "Stage":["1 Immersion","2 Participation","3 Internalisation","4 Embedding","5 Transmission"],
            "Week Range":["40–156","156–312","312–624","624–1200","1200+"],
            "Capability":["Passive absorption","Active participation","Norm internalisation","Deep embedding","Cultural transmission"],
            "Status":["✅" if w>=40 else "⏳","✅" if w>=156 else "⏳","✅" if w>=312 else "⏳","✅" if w>=624 else "⏳","✅" if w>=1200 else "⏳"],
        }
        st.dataframe(pd.DataFrame(cult_stages), use_container_width=True, hide_index=True)

    with tab2:
        locs = [("Hospital",0,0,"#f87171","Origin"),("H8 Home",1090,880,"#4ade80","Primary cultural context"),
                ("H1",1050,900,"#60a5fa","Extended family"),("H7",1100,870,"#60a5fa","Neighbour"),
                ("BeenFore City",1000,1000,"#facc15","Cultural centre")]
        fig = go.Figure()
        for name,x,y,color,role in locs:
            fig.add_trace(go.Scatter(x=[x],y=[y],mode="markers+text",text=[name],
                textposition="top center",marker=dict(size=18,color=color),name=f"{name} — {role}"))
        fig.update_layout(template="plotly_dark",height=320,xaxis_title="World X (m)",yaxis_title="World Y (m)",margin=dict(l=0,r=0,t=20,b=0))
        st.plotly_chart(fig,use_container_width=True)

    with tab3:
        npcs = {"NPC":["Lorraine","Alexis","Evelyn","James"],
                "Cultural Role":["Primary caregiver — cultural anchor","Extended family — secondary model","Peer model — play culture","Community member — neighbourhood"],
                "Bond":[0.95,0.70,0.65,0.30],
                "Cultural Influence":["Highest","High","Medium","Low"]}
        st.dataframe(pd.DataFrame(npcs), use_container_width=True, hide_index=True)
        fig2 = go.Figure(go.Bar(x=["Lorraine","Alexis","Evelyn","James"],y=[0.95,0.70,0.65,0.30],
            marker_color=["#4ade80","#60a5fa","#c084fc","#9ca3af"],text=[0.95,0.70,0.65,0.30],textposition="outside"))
        fig2.update_layout(template="plotly_dark",height=220,yaxis=dict(range=[0,1.1],title="Bond Strength"),margin=dict(l=0,r=0,t=20,b=0))
        st.plotly_chart(fig2,use_container_width=True)

    with tab4:
        df = sheet_to_df("🌍 Phase 5 — Cultural Layer")
        if df.empty:
            st.warning("Sheet data not available")
        else:
            st.dataframe(df, use_container_width=True, hide_index=True)

# ═══════════════════════════════════════════════════════════════════════════════
# PAGE: PHASE 6 — IDENTITY LAYER
# ═══════════════════════════════════════════════════════════════════════════════
elif page == "🪪 Phase 6 — Identity Layer":
    st.title("🪪 Phase 6 — Identity Layer")
    w = state.get('week', 0)
    stage = state.get('stage', 'Embryo')
    ic = state.get('identity', 0.0)
    nc = state.get('narrative', 0.0)
    # Identity stage
    if w >= 1200: id_stage = "6 — Wisdom self"
    elif w >= 936: id_stage = "5 — Value self"
    elif w >= 624: id_stage = "4 — Narrative self"
    elif w >= 312: id_stage = "3 — Social self"
    elif w >= 156: id_stage = "2 — Embodied self"
    elif w >= 40:  id_stage = "1 — Proto-self"
    else:          id_stage = "0 — Prenatal"
    st.caption(f"Tick {tick:,} · Week {w} · {stage} · Identity Stage: {id_stage}")

    col1,col2,col3,col4 = st.columns(4)
    col1.metric("Life Stage", stage)
    col2.metric("Identity Stage", id_stage)
    col3.metric("Identity Confidence IC(t)", f"{ic:.3f}")
    col4.metric("Narrative Coherence NC(t)", f"{nc:.3f}")

    tab1,tab2,tab3,tab4 = st.tabs(["🪪 Identity Schema","📊 Skill Graph","📈 Stages","📄 Full Sheet"])

    with tab1:
        st.subheader("Identity Schema Equations")
        st.code("""I(t) = [personality, values, goals, skills, narrative, bonds]
P(t+1) = P(t) + η_p·(experience − P(t))   # η_p=0.001 (very slow)
V_stable(t) = EMA(V_system(t), τ=1000)    # Value consolidation
S_k(t+1) = S_k(t) + η_s·(practice_k − S_k(t))  # Skill mastery
NC(t) = sim(self_model, episodic_history)  # Narrative coherence
IC(t) = 1 − Var(I(t), last_1000_ticks)    # Identity confidence""", language="text")
        st.subheader("Current Identity Components")
        identity_data = {
            "Component":["Episodic self-history","Stable prediction","Emotional history","Social bonds","Value system","Self-model C(t)","Skill identity","Wisdom & legacy"],
            "Status":[f"✅ {state.get('ltm',0):,} events",f"✅ Error={state.get('pred_err',1.0):.3f}","✅ H(t) active","✅ Lorraine=0.95","✅ V(s)=1.0",f"✅ C={state.get('C',0.0):.3f}",f"🟡 Motor {state.get('motor',1)}","✅ Active" if state.get('phase7',False) else "⏳ Wk 1200+"],
            "Phase":["Phase 3","Phase 4","Core","Phase 5","Phase 3","Core","Phase 3","Phase 7"],
        }
        st.dataframe(pd.DataFrame(identity_data), use_container_width=True, hide_index=True)

    with tab2:
        skills = ["Language","Logic","Spatial","Social","Emotion","Motor","Creative","Ethical","Planning","Memory","Pattern","Causal","Numerical","Narrative","Attention","Sensory","Predictive","Cultural","Self-Reg","Curiosity","Empathy","Legacy"]
        current = [0.72,0.68,0.81,0.65,0.59,0.88,0.45,0.52,0.41,0.77,0.74,0.63,0.55,0.69,0.71,0.84,0.48,0.61,0.57,0.79,0.66,0.38]
        target  = [0.95,0.90,0.85,0.95,0.90,0.90,0.85,0.90,0.88,0.85,0.88,0.90,0.85,0.88,0.90,0.88,0.90,0.88,0.92,0.85,0.95,0.90]
        fig = go.Figure()
        fig.add_trace(go.Scatterpolar(r=current+[current[0]],theta=skills+[skills[0]],fill="toself",name="Current",line_color="#60a5fa"))
        fig.add_trace(go.Scatterpolar(r=target+[target[0]],theta=skills+[skills[0]],fill="toself",name="Target",line_color="#4ade80",opacity=0.3))
        fig.update_layout(template="plotly_dark",height=400,polar=dict(radialaxis=dict(range=[0,1])),margin=dict(l=0,r=0,t=20,b=0))
        st.plotly_chart(fig,use_container_width=True)

    with tab3:
        id_stages = {"Stage":["1 Proto-self","2 Embodied","3 Social","4 Narrative","5 Value","6 Wisdom"],
                     "Week Range":["40–156","156–312","312–624","624–936","936–1200","1200+"],
                     "Milestone":["Self-other distinction","Stable body schema","Social role identity","Autobiographical self","Value-anchored identity","Wisdom & legacy identity"],
                     "Status":["✅" if w>=40 else "⏳","✅" if w>=156 else "⏳","✅" if w>=312 else "⏳","✅" if w>=624 else "⏳","✅" if w>=936 else "⏳","✅" if w>=1200 else "⏳"]}
        st.dataframe(pd.DataFrame(id_stages), use_container_width=True, hide_index=True)

    with tab4:
        df = sheet_to_df("🪪 Phase 6 — Identity Layer")
        if df.empty:
            st.warning("Sheet data not available")
        else:
            st.dataframe(df, use_container_width=True, hide_index=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    w = state.get('week', 0)
    stage = state.get('stage', 'Embryo')
    st.caption(f"Tick {tick:,} · Week {w} · {stage}")
    ...
    with tab2:
        npcs = [("Lorraine","Primary caregiver",0.95,"Nurturing"),("Alexis","Secondary family",0.70,"Warm"),
                ("Evelyn","Secondary family",0.65,"Playful"),("James","Neighbour",0.30,"Neutral")]
        fig2 = go.Figure(go.Bar(x=[n[0] for n in npcs],y=[n[2] for n in npcs],
            marker_color=["#4ade80","#60a5fa","#c084fc","#9ca3af"],text=[n[2] for n in npcs],textposition="outside"))
        fig2.update_layout(template="plotly_dark",height=240,yaxis=dict(range=[0,1.1],title="Bond Strength"),margin=dict(l=0,r=0,t=20,b=0))
        st.plotly_chart(fig2,use_container_width=True)
        df2 = sheet_to_df("👥 NPC Engine")
        if not df2.empty:
            st.dataframe(df2, use_container_width=True, hide_index=True)

    with tab3:
        df3 = sheet_to_df("🗺️ World Data")
        if not df3.empty:
            st.dataframe(df3, use_container_width=True, hide_index=True)
        else:
            st.warning("World Data sheet not available")

    with tab4:
        df4 = sheet_to_df("🏥 Immersive Places")
        if not df4.empty:
            st.dataframe(df4, use_container_width=True, hide_index=True)
        else:
            st.warning("Immersive Places sheet not available")
